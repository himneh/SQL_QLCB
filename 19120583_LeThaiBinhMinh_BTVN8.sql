...
--EXCEPT
SELECT LB.MACB
FROM LICHBAY LB JOIN MAYBAY MB ON LB.MALOAI = MB.MALOAI
WHERE NOT EXISTS
	(
...
WHERE NOT EXISTS
	...
	AND NOT EXISTS
		(
		SELECT * FROM MAYBAY MB1
		WHERE MB.MALOAI = MB1.MALOAI
		AND MB1.MALOAI = LMB.MALOAI
		)
	)
--GROUP BY
SELECT DISTINCT LB.MACB
FROM LICHBAY LB JOIN LOAIMB LMB ON LB.MALOAI = LMB.MALOAI 
JOIN MAYBAY MB ON MB.MALOAI = LMB.MALOAI AND MB.SOHIEU = LB.SOHIEU
WHERE LMB.HANGSX = 'Boeing'
GROUP BY LB.MACB
HAVING COUNT(DISTINCT LB.MALOAI) = (SELECT COUNT(DISTINCT MB1.MALOAI) 
...
WHERE NOT EXISTS 
	...
	EXCEPT
	SELECT KN2.MALOAI
	FROM KHANANG KN2 JOIN NHANVIEN NV1 ON NV1.MANV = KN2.MANV
	)
GO
...
FROM NHANVIEN NV JOIN KHANANG KN ON NV.MANV = KN.MANV
JOIN LOAIMB LMB ON KN.MALOAI = LMB.MALOAI AND LMB.HANGSX = 'Airbus'
GROUP BY NV.MANV,NV.TEN
HAVING COUNT (DISTINCT KN.MALOAI) =  (SELECT COUNT(DISTINCT KN1.MALOAI)  
									FROM KHANANG KN1 JOIN LOAIMB LMB1 ON KN1.MALOAI = LMB1.MALOAI AND LMB1.HANGSX = 'Airbus')
GO
...
AND LB.MACB = '100' AND NV.LOAINV = 0
GROUP BY PC.MANV,NV.TEN
HAVING COUNT(DISTINCT PC.NGAYDI) = (SELECT COUNT(*) 
									FROM LICHBAY LB1 
									WHERE LB1.MACB = '100'
									)
GO
...
FROM LICHBAY LB JOIN LOAIMB LMB ON LB.MALOAI = LMB.MALOAI AND LMB.HANGSX = 'Boeing' 
AND NOT EXISTS (
		(
		SELECT MALOAI 
		FROM LOAIMB 
...
AND NOT EXISTS (
		...
		SELECT LB1.MALOAI 
		FROM LICHBAY LB1
		WHERE LB1.NGAYDI = LB.NGAYDI AND LMB1.MALOAI = LB1.MALOAI
		)
		)
)
GO
...
--GROUP BY
SELECT DISTINCT LB.NGAYDI 
FROM LICHBAY LB JOIN MAYBAY MB ON MB.MALOAI = LB.MALOAI
JOIN LOAIMB LMB ON LMB.MALOAI = MB.MALOAI AND LMB.HANGSX = 'Boeing'
GROUP BY LB.NGAYDI
HAVING COUNT(DISTINCT MB.MALOAI) = (SELECT COUNT(DISTINCT MB1.MALOAI) 
									FROM MAYBAY MB1 JOIN LOAIMB LMB1 ON LMB1.MALOAI = MB1.MALOAI
									AND LMB1.HANGSX = 'Boeing')
GO
...
--GROUP BY
SELECT LMB.MALOAI 
FROM LOAIMB LMB JOIN LICHBAY LB ON LMB.MALOAI = LB.MALOAI AND LMB.HANGSX = 'Boeing' 
GROUP BY LMB.MALOAI
...
FROM KHACHHANG KH JOIN DATCHO DCC ON KH.MAKH = DCC.MAKH
WHERE NOT EXISTS (
				  (SELECT DC.NGAYDI 
				  FROM DATCHO DC
				  WHERE DC.NGAYDI BETWEEN '1/1/2000' AND '10/31/2000')
				  EXCEPT
				  (SELECT DC1.NGAYDI 
				  FROM DATCHO DC1
				  WHERE DC1.MAKH = DCC.MAKH)
				  )
GO
...
WHERE NOT EXISTS (
				  ...
				  (SELECT *
				  FROM DATCHO DC1
				  WHERE DC1.MAKH = DCC.MAKH AND DC1.NGAYDI = DC.NGAYDI
				  )
...
--GROUP BY 
SELECT DISTINCT KH.MAKH,KH.TEN 
FROM KHACHHANG KH JOIN DATCHO DC ON KH.MAKH = DC.MAKH AND DC.NGAYDI BETWEEN '1/1/2000' AND '10/31/2000' 
GROUP BY KH.MAKH,KH.TEN
HAVING COUNT (DISTINCT DC.NGAYDI) = (SELECT COUNT(DISTINCT DC1.NGAYDI) 
								FROM KHACHHANG KH1 JOIN DATCHO DC1 ON KH1.MAKH = DC1.MAKH 
								AND DC1.NGAYDI BETWEEN '1/1/2000' AND '10/31/2000')
GO

--Q57: Cho biết mã và tên phi công có không khả năng lái tất cả các máy bay của hãng Airbus
--EXCEPT
SELECT NV.MANV, NV.TEN
FROM NHANVIEN NV
...
WHERE NV.LOAINV = 1 AND EXISTS (
		...
		(SELECT KN.MALOAI	
        FROM KHANANG KN 
		WHERE KN.MANV = NV.MANV AND KN.MALOAI = LMB1.MALOAI)
        )  
GO
...
HAVING COUNT(DISTINCT LB.MALOAI) = 
	(
	SELECT COUNT(DISTINCT CB1.SBDI)
	FROM CHUYENBAY CB1 JOIN LICHBAY LB1 ON CB1.MACB = LB1.MACB
	JOIN LOAIMB LMB1 ON LB1.MALOAI = LMB1.MALOAI
	AND LMB1.HANGSX = 'Boeing'
	)
GO
